...
io.on('connection', (socket) => {
  ...
  socket.on('showAnswer', ({ quizId, answer }) => {
    if (quizzes[quizId] && quizzes[quizId].started) {
      console.log(`Réponse pour le quiz ${quizId} :`, answer);

      // Diffuser la réponse uniquement à la room correspondante
      io.to(quizId).emit('showAnswer', {
        type: 'answer',
        data: answer,
      });
    }
